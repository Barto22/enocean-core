...
/*
 * Instantiate a context named usbd using the default USB device
...
USBD_DEVICE_DEFINE(usbd, DEVICE_DT_GET(DT_NODELABEL(zephyr_udc0)),
                   CONFIG_COMMON_USBD_VID, CONFIG_COMMON_USBD_PID);
/* doc device instantiation end */

/* doc string instantiation start */
USBD_DESC_LANG_DEFINE(lang);
USBD_DESC_MANUFACTURER_DEFINE(mfr, CONFIG_COMMON_USBD_MANUFACTURER);
USBD_DESC_PRODUCT_DEFINE(product, CONFIG_COMMON_USBD_PRODUCT);
...
static const uint8_t attributes =
    (IS_ENABLED(CONFIG_COMMON_USBD_SELF_POWERED) ? USB_SCD_SELF_POWERED : 0) |
    (IS_ENABLED(CONFIG_COMMON_USBD_REMOTE_WAKEUP) ? USB_SCD_REMOTE_WAKEUP : 0);

/* Full speed configuration */
USBD_CONFIGURATION_DEFINE(sample_fs_config, attributes,
                          CONFIG_COMMON_USBD_MAX_POWER, &fs_cfg_desc);

/* High speed configuration */
USBD_CONFIGURATION_DEFINE(sample_hs_config, attributes,
                          CONFIG_COMMON_USBD_MAX_POWER, &hs_cfg_desc);
/* doc configuration instantiation end */

#if CONFIG_COMMON_USBD_20_EXTENSION_DESC
/*
...
struct usbd_context* sample_usbd_setup_device(usbd_msg_cb_t msg_cb) {
    ...
    IF_ENABLED(CONFIG_HWINFO, (err = usbd_add_descriptor(&usbd, &sample_sn);))
    if (err) {
